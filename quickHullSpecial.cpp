...
class cdll{
    ...
    void insert(vector<long long> x , vector<long long> y){
        long long  val, val2;
        val = x[0], val2 = y[0];
        root = new node;
        main_root = root;
    ...
    void push(long long val, long long val2){
        node* temp1 = new node;
        temp1->data = val;
        temp1->data2 = val2;
...
struct hull{
    vector<long long> x_cord;
    vector<long long> y_cord;
};
...
class convex_hull{
    ...
    hull con_hull(vector<long long> x, vector<long long> y, 
				  long long start, long long end, long long num) {
    	if (start < end ){
    ...
    int equation(long long x1, long long x2, long long y1, 
				 long long y2, long long x, long long y){
        return ((y-y1) * (x1-x2)) - ((y1-y2)*(x-x1));
    }

    bool check(point l, point r, hull lef, hull rig){
        bool f;
        long long tempo1 = lef.x_cord[0];
        long long tempo2 = lef.y_cord[0];
        for (long long i = 0; i < lef.x_cord.size(); i++){
        ...
        for(long long i = 0; i < rig.x_cord.size(); i++ ){
...
int main()
{
	...
	freopen("guickHullSpecialAns.txt","w+",stdout);
	for(long long k=0;k<5;++k){
		...
		cout<<"Input Size: "<<h1.n<<endl;
		cout<<"Convex Hull SIze: "<<ans.n<<endl;
